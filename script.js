...
*/
/*
  WHY these brightness values:
  blur(16px) + contrast(18) pumps up whatever alpha/brightness is there.
  Colors brighter than ~#bbbbbb will blow out to white after the filter.
  Rule of thumb: keep hex values below ~#aa in each channel.
  Fire works because its oranges are mid-range (e8=232 but heavily RED,
  low GREEN/BLUE keeps it from going white). Other modes need the same logic.
*/
const MODES = {
    fire:     { colors: ["#e8927c","#e8b87c","#e8c97c","#d4856e","#e8a87c","#c97a6e"], speed: 1.6 },
    pastel:   { colors: ["#b07898","#7888aa","#78aa90","#aa9870","#9478aa","#7898aa"], speed: 0.8 },
    electric: { colors: ["#5899cc","#5877cc","#6688bb","#7799bb","#5888bb","#6677bb"], speed: 2.2 },
    calm:     { colors: ["#3d6e99","#3d7e99","#3d708e","#487899","#3a6e88","#4a7899"], speed: 0.42 },
    aurora:   { colors: ["#5aaa80","#8a5aaa","#5aaa99","#aa5a80","#7aaa5a","#aa805a"], speed: 1.0 },
}
...
// Klik tombol: toggle panel & play/pause
function toggleMusic() {
    volPanelOpen = !volPanelOpen
    volWrap.classList.toggle("open", volPanelOpen)
    if (!musicPlaying) {
        bgMusic.play().then(() => {
            musicPlaying = true
...
// Tutup panel kalau klik di luar area music-ui
document.addEventListener("click", (e) => {
    if (!document.getElementById("music-ui").contains(e.target)) {
        volPanelOpen = false
